$(document).ready(function () {
  hide_loading();
  showAlert(
    'Please Enter your Voter Card Number to Verify yourself in order to cast your pricelsess vote.',
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.success) {
        ...
        showAlert(
          'Successfully Verified, Vote Now!',
          'rgba(136, 255, 156, 0.3)',
          'rgb(0, 128, 0)'
        );
        setTimeout(function () {
          showAlert(
...
function showShortDetails() {
  $('.basic-details').html(
    candidate_details_json.uuid + ' (' + candidate_details_json.name + ')'
  );
